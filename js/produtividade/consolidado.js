    ...
    processarDados: function(data) {
        ...
        const ranking = assistentes.map(u => {
            ...
            return {
                ...
                atingimento: (Number(u.meta_producao) * Number(u.total_dias_uteis)) > 0 ? (Number(u.total_qty) / (Number(u.meta_producao) * Number(u.total_dias_uteis)) * 100) : 0
            };
        });
    ...
    renderizarGrafico: function(dados) {
        ...
        this.chartInstance = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [
                    { label: 'Produção Real', data: valores, backgroundColor: cores, borderRadius: 4, order: 2 },
                    { label: 'Meta Esperada', data: metas, type: 'line', borderColor: '#94a3b8', borderWidth: 2, borderDash: [5, 5], pointRadius: 0, order: 1, tension: 0.4 }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: true, position: 'bottom' }, tooltip: { callbacks: { label: function(context) { let label = context.dataset.label || ''; if (label) { label += ': '; } if (context.parsed.y !== null) { label += context.parsed.y.toLocaleString('pt-BR'); } return label; } } } },
