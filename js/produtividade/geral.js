...
Produtividade.Geral = {
    ...
    // Lista de Feriados (Strings ISO para performance)
    feriados: {
    ...
    // Status que contam como esforço (Volume) mas são neutros em qualidade
    statusNeutros: ['DUPL', 'EMPR', 'IA', 'NA', 'N/A', 'REVALIDA', 'CANCELADO', 'JUSTIFICADO'],

    init: function() { 
        console.log("🔧 [NEXUS] Produtividade Geral: Sistema Iniciado.");
        this.carregarTela(); 
    ...
    // Sanitização de Porcentagem (Engine Core)
    parsePercentage: function(val) {
        if (val === null || val === undefined || val === '') return null;
        // Remove %, troca vírgula por ponto e remove espaços
    ...
    carregarTela: async function() {
        ...
        tbody.innerHTML = '<tr><td colspan="12" class="text-center py-12 text-slate-400"><i class="fas fa-satellite-dish fa-spin mr-2"></i> Sincronizando métricas de alta precisão...</td></tr>';

        try {
            // 1. Busca Paralela com RANGE EXPANDIDO (Correção do Bug de Paginação)
            // O range(0, 50000) garante que leremos todos os registros, superando o limite padrão de 1000.
            const [resProd, resAudit, resUsers, resMetas] = await Promise.all([
                ...
                // Usuários
                Sistema.supabase.from('usuarios')
                ...
                Sistema.supabase.from('metas')
                    ...
                    .eq('mes', parseInt(dataInicio.split('-')[1]))
                    .eq('ano', parseInt(dataInicio.split('-')[0]))
            ]);

            if (resProd.error) throw resProd.error;
            if (resAudit.error) console.warn("[NEXUS] Aviso Auditoria:", resAudit.error);

            // 2. Indexação de Dados Auxiliares (Mapas para O(1))
            ...
            // 3. Processamento de Auditoria (Cálculo Isolado)
            const mapaAuditoria = {};     
            if (resAudit.data) {
                resAudit.data.forEach(a => {
                    const val = this.parsePercentage(a.porcentagem);
                    if (val === null) return; // Ignora registros inválidos
            ...
            // 4. Consolidação dos Dados (Merge)
            let dadosAgrupados = {};
            ...
            resProd.data.forEach(item => {
                ...
                // Define o que conta para volume
            ...
            // B) Adiciona usuários que só têm Auditoria (sem produção no período)
    ...
    inicializarUsuario: function(grupo, uid, mapaUsuarios, mapaMetas, mapaAuditoria) {
        ...
        grupo[uid] = {
            usuario: userObj, 
            registros: [], // Placeholder se precisar de detalhes
    ...
    renderizarTabela: function() {
        ...
        // Filtro de Perfil (Remove Gestão se checkbox unchecked)
        ...
        lista.forEach(d => {
            ...
            // --- LÓGICA DE APRESENTAÇÃO DA ASSERTIVIDADE ---
            let assertDisplay = "-";
            ...
            if (d.auditoria.qtd > 0) {
                ...
                // Semáforo de Cores (Traffic Light System)
                if (valor >= 98) {
                    corAssert = "text-emerald-700 font-bold bg-emerald-50 border-emerald-200";
                } else if (valor >= 95) {
                    corAssert = "text-blue-700 font-bold bg-blue-50 border-blue-200";
                } else if (valor >= 90) {
                    corAssert = "text-amber-700 font-bold bg-amber-50 border-amber-200";
                } else {
                    corAssert = "text-rose-700 font-bold bg-rose-50 border-rose-200";
                }
            }
