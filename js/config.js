...
// Verifica se a biblioteca do Supabase foi carregada antes
if (typeof supabase === 'undefined') {
    console.error("ERRO CRÍTICO: A biblioteca do Supabase (CDN) não foi carregada antes do config.js!");
    // Opcional: alert("Erro de sistema: Biblioteca do Supabase não carregada.");
} else {
    // IMPORTANTE: Atribui explicitamente ao window para garantir acesso global em todos os arquivos
    window._supabase = supabase.createClient(SB_URL, SB_KEY);
    console.log("Supabase conectado e atribuído ao window._supabase com sucesso!");
}