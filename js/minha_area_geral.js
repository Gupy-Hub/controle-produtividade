// Objeto Responsável pelo Diário e KPIs
const MA_Diario = {
    ...
    atualizarKPIs: function(dados) {
        ...
        let metaTotal = 0;
        if(dados.length > 0) dados.forEach(d => metaTotal += (d.meta_ajustada || d.meta_diaria || 650)); 
    ...
    atualizarTabela: function(dados, viewingTime) {
        ...
        dados.forEach(item => {
            ...
            if(MA_Main.isMgr && !viewingTime && item.id) {
                inputMeta = `<input type="number" value="${meta}" onchange="MA_Diario.atualizarMetaBD(${item.id}, this.value, ${meta})" class="w-20 text-center border border-slate-200 rounded px-1 py-1 text-xs font-bold bg-slate-50 focus:bg-white outline-none focus:ring-2 focus:ring-blue-200">`;
            }
...
// Objeto Responsável por Gráficos
const MA_Evolucao = {
    chart: null,
...
// Objeto Responsável pelo Comparativo
...
// Objeto Responsável pelo Feedback
const MA_Feedback = {
