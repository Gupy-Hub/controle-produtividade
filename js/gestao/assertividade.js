Gestao.Assertividade = {
    timerBusca: null, 
    dadosCache: [], // Armazena os dados originais trazidos do servidor
    ...
    executarBuscaRPC: async function(termo) {
        ...
        try {
            ...
            // Formata e Guarda no Cache
            this.dadosCache = (data || []).map(item => ({
                ...item,
                usuarios: { nome: item.usuario_nome }
            }));

            // Popula os Selects de Filtro (Status e Auditora)
    ...
    popularSelectsFiltro: function() {
        ...
        const auditorasUnicas = new Set();

        this.dadosCache.forEach(item => {
            if(item.status) statusUnicos.add(item.status.toUpperCase());
            if(item.auditora) auditorasUnicas.add(item.auditora);
        });

        // Monta HTML Status
        let htmlStatus = '<option value="">Todos</option>';
        Array.from(statusUnicos).sort().forEach(st => {
        ...
        Array.from(auditorasUnicas).sort().forEach(au => {
    ...
    aplicarFiltrosLocais: function() {
        ...
        const fStatus = document.getElementById('filtro-status')?.value.toUpperCase();
        const fObs = document.getElementById('filtro-obs')?.value.toLowerCase();
        const fAuditora = document.getElementById('filtro-auditora')?.value.toLowerCase(); // Auditora pode ter case mix
        ...
        const filtrados = this.dadosCache.filter(item => {
            ...
            // Status (Match exato se selecionado)
            if (fStatus && (item.status || '').toUpperCase() !== fStatus) return false;

            // Obs
            if (fObs && !(item.observacao || '').toLowerCase().includes(fObs)) return false;

            // Auditora (Match exato se selecionado)
            if (fAuditora && (item.auditora || '').toLowerCase() !== fAuditora) return false;

            return true;
        });

        this.renderizarTabela(filtrados, "Filtrado localmente");
    },
    ...
    renderizarTabela: function(lista, mensagemRodape = "") {
        ...
        if (lista.length === 0) {
            tbody.innerHTML = '<tr><td colspan="12" class="text-center py-12 text-slate-400"><div class="flex flex-col items-center gap-2"><i class="fas fa-filter text-3xl opacity-20"></i><span>Nenhum registro com esses filtros.</span></div></td></tr>';
            return;
        }
        ...
        lista.forEach(item => {
            ...
            // Status Badge (Lógica de Cores da Planilha)
